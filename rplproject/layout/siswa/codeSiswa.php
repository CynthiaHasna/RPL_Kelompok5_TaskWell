...
// Cek apakah user login dan ada kode_kelas dikirim
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (!isset($_SESSION['id_user'])) {
        $message = "Silakan login terlebih dahulu.";
    } elseif (empty($_POST['kode_kelas'])) {
        $message = "Kode kelas tidak boleh kosong.";
    } else {
        $kode_masuk = mysqli_real_escape_string($koneksi, $_POST['kode_kelas']);
        $id_user = $_SESSION['id_user'];

        // Cek apakah kode kelas ada
        $query = "SELECT * FROM kelas WHERE kode_kelas = '$kode_masuk'";
        ...
        if ($kelas) {
            ...
            $check = mysqli_query($koneksi, "SELECT * FROM kelas_anggota WHERE id_kelas = $id_kelas AND id_user = $id_user");
            if (mysqli_num_rows($check) > 0) {
                $message = "Kamu sudah tergabung di kelas ini.";
            } else {
                // Insert ke kelas_anggota
                $insert = mysqli_query($koneksi, "INSERT INTO kelas_anggota (id_kelas, id_user) VALUES ($id_kelas, $id_user)");
                if ($insert) {
                    header("Location: Beranda_siswa.php");
                    exit();
                } else {
                    $message = "Gagal join kelas: " . mysqli_error($koneksi);
                }
            }
        } else {
...
<body style="cursor: url('./../Homes/Asset/cursor.png') 0 0, auto;">
  ...
  <div class="popup-container">
    <img src="../../gambar/Capybara.png" alt="Capybara Wisuda" />
    <p>Task well menemani kamu untuk membuat kelas dan<br>menciptakan kelas yang menyenangkan!</p>
    <form method="POST" action="">
      <input type="text" name="kode_kelas" class="class-code" placeholder="Contoh: AB123XYZ" required />
      <br>
      <button type="submit" class="btn">Gabung</button>
    </form>

    <?php if ($message != ""): ?>
      <div class="me ssage"><?= htmlspecialchars($message) ?></div>
    <?php endif; ?>
  </div>
  ...
  <script>
    ...
    function lanjutkanMasukKelas() {
    const kode = document.getElementById('kodeKelasInput').value.trim();
    if (kode === "") {
      alert("Silakan masukkan kode kelas terlebih dahulu.");
      return;
    }

    alert("Kode berhasil masuk!");
    setTimeout(() => {
      window.location.href = "Beranda_siswa.php"; // Ganti sesuai halaman tujuan
    }, 1000); // 1000 = 1 detik delay
  }
